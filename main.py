...
async def main():
    ...
    while running:
        ...
        # draw maze
        scaled = pygame.transform.scale(maze, (maze.get_width()*3, maze.get_height()*3))
        screen.blit(scaled, maze_rect)

        # draw ghosts
        scaled = pygame.transform.scale(blinky.sprites[0], (blinky.sprites[0].get_width()*3, blinky.sprites[0].get_height()*3))
        ...
        for p in range(len(points)):
            col = p/len(points) * 255 + 10
            pygame.draw.circle(screen, (col, 0, col), points[p], 5, 2)
